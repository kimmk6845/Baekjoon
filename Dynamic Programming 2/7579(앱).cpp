...
int main()
{
    ...
    int N, M;   // M: 확보해야 하는 메모리 크기
    cin >> N >> M;

    vector<int> m(N + 1);   // 메모리
    vector<int> c(N + 1);   // 비용
    for (int i = 1; i < N + 1; i++)
        cin >> m[i];
    for (int i = 1; i < N + 1; i++)
    ...
    int result = INT_MAX;
    for (int i = 1; i < N + 1; i++)  // 앱 idx
    {
        for (int j = 0; j < sum + 1; j++)   // 비용
        {
            if (j >= c[i])
...
현재 실행 중이 아니더라도 이렇게 메모리에 남겨두는 이유는 사용자가 이전에 실행하던 앱을 다시 불러올 때에 직전의 상태를 메인 메모리로부터 읽어들여 실행 준비를 빠르게 마치기 위해서이다.
